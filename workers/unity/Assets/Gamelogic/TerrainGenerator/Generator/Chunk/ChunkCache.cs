...
namespace TerrainGenerator
{
    public class ChunkCache
    {
        ...
        public bool ChunkCanBeAdded(int x, int z)
        {
            ...
            return  // if the coordinate is not found in any of the three queues, then it must be new, hence can be added, i.e. without repetition
                !(
        ...
        {
            var key = new Vector2i(x, z);
            return  // if the coordinate is in any of the three queues, then it must be old, hence can be removed
                RequestedChunks.ContainsKey(key)
                || ChunksBeingGenerated.ContainsKey(key)
                || LoadedChunks.ContainsKey(key);
        }
        ...
        {
            // use a separate thread for each heightmap generation
        ...
        {
            // flag if any new terrain being accepted in this call
            var anyTerrainCreated = false;

            var chunks = ChunksBeingGenerated.ToList();
            foreach (var chunk in chunks)
            {
                // check that: the thead of execution is finished, but the terrain is not generated yet
